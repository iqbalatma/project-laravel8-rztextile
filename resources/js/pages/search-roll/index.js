...
$(document).ready(function () {
    ...
    function onChangeSelectize(value) {
        const rollId = value;
        const dataSet = $(`.item-roll-selectized[data-id="${rollId}"]`).data(
            "data"
        );
        ...
        $.ajax({
            ...
            .done(function (response) {
                ...
                if (response.status == 200) {
                    ...
                    if (data.unit.name) {
                        $("#roll-quantity-unit").text(
                            data.quantity_unit + " " + data.unit.name ?? ""
                        );
                    } else {
                        $("#roll-quantity-unit").text(data.quantity_unit);
                    }
                    $("#roll-qrcode").text(data.qrcode);
                    $("#roll-basic-price").text(
                        helper.formatIntToRupiah(data.basic_price)
                    );
                    $("#roll-selling-price").text(
                        helper.formatIntToRupiah(data.selling_price)
                    );

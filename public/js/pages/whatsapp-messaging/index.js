  ...
  $("#segmentation_id").on("change", function () {
    ...
    $.ajax({
      ...
      if (dataPromotionMessages.length > 0) {
        ...
        $("#promotion-blast").append("<option selected disabled>Select Promotion Message Bellow</option>");
        dataPromotionMessages.forEach(function (element) {
          $("#promotion-blast").append("<option value=\"".concat(element.id, "\" data-message=\"").concat(element.message, "\">").concat(element.name, "</option>"));
        });
      } else {
